...
export default function ArticleDetail() {
  ...
  useEffect(() => {
    ...
    fetchOnePost({ id: Number.parseInt(id as string) })
      .then(response => {
        if (response?.data) {
          const contentHtml = decodeUtils.decodeHtml(response.data.content_html || "文章内容为空！")
  ...
  if (error || !post?.contentHtml) {
    return (
      <ErrorCard
        ...
        message={error ? error.message : "哇，这里好像很安静！也许你可以成为第一个留言的人。"}
      />
    )
  }
    ...
    <div className="container mx-auto px-4">
      <div className="relative mb-6">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 text-center">
          <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-foreground drop-shadow-sm">{post.title}</h1>
        </div>
      </div>
