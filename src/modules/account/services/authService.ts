 ...
 * 邮箱密码登录
 */
export const loginUser = (data: LoginFormValues): Promise<HttpResponse<{
    access_token: string;
 ...
 * 注册新账户
 */
export const registerUser = (data: RegisterFormValues): Promise<HttpResponse<{
    email: string;
    nickname: string;
}>> => {
 ...
 * 获取账户信息
 */
export const getAccountInfo = (email: string): Promise<HttpResponse<{
    avatar: string;
    email: string;
    nickname: string;
    phone: string;
 ...
 * 重置用户密码
 */
export const resetUserPassword = (data: ResetPasswordFormValues): Promise<HttpResponse<{
    success: boolean;
}>> => {
